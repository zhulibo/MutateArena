...
#include "MutateArena/Maps/TeleportPortal.h"
...
{
	// 【核心拦截】：如果是传送门，直接移交业务并拦截打墙逻辑
	if (ATeleportPortal* Portal = Cast<ATeleportPortal>(OtherActor))
	{
		// 抢救可能因撞击瞬间被底层归零的速度，用于传送后的动能维持
		if (ProjectileMovement->Velocity.IsNearlyZero())
		{
			ProjectileMovement->Velocity = GetActorForwardVector() * ProjectileMovement->InitialSpeed;
		}

		// 调用传送门封装完善的公共冷却与数学转换核心
		if (Portal->TryTeleportActor(this))
		{
			// 规避飞出时由于极近距离发生相互卡死和无限套娃
			CollisionBox->IgnoreActorWhenMoving(Portal, true);
			CollisionBox->IgnoreActorWhenMoving(Portal->TargetPortal, true);

			// 复用木板穿透机制相同的 NextTick 唤醒神技，绕过物理阻挡停滞
			GetWorld()->GetTimerManager().SetTimerForNextTick(FTimerDelegate::CreateUObject(this, &ThisClass::RestoreVelocity, ProjectileMovement->Velocity));
		}
		
		// 只要撞击的是传送门，绝不向下进行打墙、生成弹孔和特效的计算
		return;
	}
...
void AProjectileBullet::OnBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex,
...
void AProjectileBullet::OnEndOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
